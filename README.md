...
Доступны инструменты `location_search_context`, `korea_place_search`,
`korea_geocode`, `korea_reverse_geocode`, `korea_route`,
`korea_shopping_search` и `korea_kakao_links`. `location_search_context`
подготавливает приблизительный район для обычного веб-поиска, а
`korea_shopping_search` не вызывает API и только готовит переход к поиску через
локальный browser. KakaoMap-ссылки строятся по
...
живут только в RAM процесса ограниченное время и исчезают по TTL/перезапуску; в
память и ответы они не записываются. Telegram как внешний сервис всё равно
получает и хранит отправленную ему геометку — это не скрывает координаты от
самого Telegram.

Для обычного Google/DDGS/Tavily- или browser-поиска инструмент
`location_search_context` один раз преобразует токен только в название
административного района и готовую строку `localized_query`. Улица, дом, индекс
и исходные координаты отбрасываются до возврата результата. Такой контекст
можно переиспользовать в нескольких веб-запросах текущей задачи, но это лишь
приблизительная привязка к району, а не доказательство, что найденное место
ближайшее. Для точного поиска рядом и маршрутов остаются Korea-tools.

Для такого токена обычный reverse geocode текущей точки по-прежнему заблокирован,
чтобы точный адрес не попал в историю. Поиск рядом возвращает только крупный
диапазон расстояния и относительный ранг, а не точные расстояния до нескольких
мест; выбранный маршрут может вернуть необходимую пользователю оценку пути, не
раскрывая origin или геометрию трека.
